      ...
      db.get(dupeCheck, [], function (err, row) {
        if (err) {
          ...
          if (row && filterDupes) {
            ...
            db.get("SELECT id, ignore, push, pushpri, pushgroup, pushsound, telegram, telechat, twitter, twitterhashtag, discord, discwebhook, shellenable, shellcmd, mailenable, mailto FROM capcodes WHERE ? LIKE address ORDER BY REPLACE(address, '_', '%') DESC LIMIT 1", address, function(err,row) {
...
router.post('/capcodes', function(req, res, next) {
  ...
  if (req.body.address && req.body.alias) {
    ...
    db.serialize(() => {
      ...
      db.run("REPLACE INTO capcodes (id, address, alias, agency, color, icon, ignore, push, pushpri, pushgroup, pushsound, telegram, telechat, twitter, twitterhashtag, discord, discwebhook, shellenable, shellcmd, mailenable, mailto) VALUES ($mesID, $mesAddress, $mesAlias, $mesAgency, $mesColor, $mesIcon, $mesIgnore, $mesPush, $mesPushPri, $mesPushGroup, $mesPushSound, $mesTelegram, $mesTeleChat, $mesTwitter, $mesTwitterHashTag, $mesDiscord, $mesDiscWebhook, $shellEnable, $shellCmd, $MailEnable, $MailTo );", {
