...
// defaults
var initData = {};
initData.limit = config.get('messages:defaultLimit');
initData.replaceText = config.get('messages:replaceText');
...
// auth variables
var HideCapcode = config.get('messages:HideCapcode');
var apiSecurity = config.get('messages:apiSecurity');
...
router.route('/messages')
        .get(authHelper.isLoggedInMessages, function(req, res, next) {
                config.load();
                console.time('init');
                var pdwMode = config.get('messages:pdwMode');
                var adminShow = config.get('messages:adminShow');
                var maxLimit = config.get('messages:maxLimit');
                var defaultLimit = config.get('messages:defaultLimit');
                var HideCapcode = config.get('messages:HideCapcode');
        ...
        .post(authHelper.isAdmin, function(req, res, next) {
                ...
                if (req.body.address && req.body.message) {
                        var dbtype = config.get('database:type');
                        var filterDupes = config.get(
                                'messages:duplicateFiltering'
                        );
                        var dupeLimit =
                                config.get('messages:duplicateLimit') || 0; // default 0
                        var dupeTime =
                                config.get('messages:duplicateTime') || 0; // default 0
                        var pdwMode = config.get('messages:pdwMode');
                        var adminShow = config.get('messages:adminShow');
                        var data = req.body;
                        data.pluginData = {};
                        ...
                        res.status(500).json({
                                message: 'Error - address or message missing',
                        });
                }
        });

router.route('/messages/:id').get(authHelper.isLoggedInMessages, function(
        req,
        res,
        next
) {
...
router.route('/messageSearch').get(authHelper.isLoggedInMessages, function(
        req,
        res,
        next
) {
        ...
        db.select(
                ...
                .then(rows => {
                        ...
                        if (rowCount > 0) {
                                ...
                                initData.pageCount = Math.ceil(
                                        initData.msgCount / initData.limit
                                );
                                if (initData.currentPage > initData.pageCount) {
                                        initData.currentPage = 0;
                                }
                                initData.offset =
                                        initData.limit * initData.currentPage;
                                if (initData.offset < 0) {
                                        initData.offset = 0;
                                }
                                initData.offsetEnd =
                                        initData.offset + initData.limit;
                                var limitResults = result.slice(
                                        initData.offset,
                                        initData.offsetEnd
                                );
...
router.route('/capcodes')
        ...
        .post(authHelper.isAdmin, function(req, res, next) {
                config.load();
                var updateRequired = config.get(
                        'database:aliasRefreshRequired'
                );
                if (req.body.address && req.body.alias) {
                        ...
                        var pluginconf =
                                JSON.stringify(req.body.pluginconf) || '{}';
                        db.from('capcodes')
                                .where('id', '=', id)
                                .modify(function(queryBuilder) {
                                        if (id == null) {
                        ...
                        logger.main.debug(
                                util.format('%o', req.body || 'no request body')
                        );
                } else {
                        res.status(500).json({
                                message: 'Error - address or alias missing',
                        });
                }
        });

router.route('/capcodes/agency').get(authHelper.isAdmin, function(
        req,
        res,
        next
) {
...
router.route('/capcodes/agency/:id').get(authHelper.isAdmin, function(
        req,
        res,
        next
) {
...
router.route('/capcodes/:id')
        ...
        .post(authHelper.isAdmin, function(req, res, next) {
                ...
                if (id == 'deleteMultiple') {
                        ...
                        if (!idList.some(isNaN)) {
                                ...
                                res.status(500).send({
                                        status: 'id list contained non-numbers',
                                });
                        }
                } else {
                        if (req.body.address && req.body.alias) {
                                ...
                                db.from('capcodes')
                                        ...
                                        .then(result => {
                                                ...
                                                if (updateAlias == 1) {
                                                        ...
                                                        var specificRefresh = config.get(
                                                                'global:SpecificAliasRefresh'
                                                        );
                                                        if (
                                                                specificRefresh &&
                                                                /^\d+$/.test(
                                ...
                                logger.main.debug(
                                        util.format(
                                                '%o',
                                                req.body || 'request body empty'
                                        )
                                );
                        } else {
                                res.status(500).json({
                                        message:
                                                'Error - address or alias missing',
                                });
                        }
                }
        })
        .delete(authHelper.isAdmin, function(req, res, next) {
                ...
                logger.main.debug(
                        util.format('%o', req.body || 'request body empty')
                );
        });

router.route('/capcodeCheck/:id').get(authHelper.isAdmin, function(
        req,
        res,
        next
) {
...
router.route('/capcodeRefresh').post(authHelper.isAdmin, function(
        req,
        res,
        next
) {
...
router.route('/capcodeExport').post(authHelper.isAdmin, function(
        req,
        res,
        next
) {
...
router.route('/capcodeImport').post(authHelper.isAdmin, function(
        req,
        res,
        next
) {
        ...
        converter
                ...
                .then(async data => {
                        ...
                        if ('address' in header && 'alias' in header) {
                                ...
                                config.set('database:aliasRefreshRequired', 1);
                                config.save();
                        } else {
...
router.route('/userCheck/username/:id').get(authHelper.isAdmin, function(
        req,
        res,
        next
) {
...
router.route('/userCheck/email/:id').get(authHelper.isAdmin, function(
        req,
        res,
        next
) {
        ...
        .post(authHelper.isAdmin, function(req, res, next) {
                ...
                if (id == 'deleteMultiple') {
                        ...
                        if (!idList.some(isNaN)) {
                                ...
                                res.status(400).send({
                                        status: 'error',
                                        error: 'id list contained non-numbers',
                                });
                        }
                } else {
                        if (
